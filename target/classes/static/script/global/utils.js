...
define(function(require, exports, module) {
    var Utils = {
        ...
        initPage: function(pageId) {
            ...
            page.findListId = function(obj) {
                return $(this.id).parent().find("[listid='"+obj+"']")
            }
            return page;
        },
        /**
        ...
        eventTrClickIfChanged: function(obj, callback) {
            ...
            obj.find("tr").bind('click', function(event) {
                if ($(this).parent()[0].tagName == "THEAD") {} else {
        ...
        initStep: function(obj) {
            var self = this;
            $(obj).find("div[data-steps='1']").removeClass('hide').siblings().addClass('hide');
            $(obj).find("[data-gostep]").unbind();
        ...
        getCheckboxCheckedRow: function(obj) {
            var _obj = obj.find("input[type='checkbox']:checked").not("[name='selectAll']");
            if (_obj.length == 0) {
                window.XMS.msgbox.show('请先选择一行数据！', 'info', 2000);
                return;
            }
         ...
         * @method obj 表单父元素
         */
        setSelectData: function(obj, other, callback, tag, valueCallback) {
            var self = this;
            ...
            obj.on("change", "select[data-subname]", function() {
                ...
                if (_subname) {
                    ...
                    if (suburl) {
                        if (tag) {
                            self.setSelectHtml(_thisSub, suburl, subcmd, valueCallback);
                        } else {
            ...
            obj.on("change", "select[data-complexsubname]", function() {
                var _this = $(this);

                // 判断如果有异步子项，且入参有两个以上，统一做处理;lizw20220510
                var _complexsubname = _this.data("complexsubname");
                if (_complexsubname) {
                    var _thiscomplexSub = obj.find("select[name=" + _complexsubname + "]");
                    var complexsuburl = _thiscomplexSub.data("complexsuburl");
                    console.log('lizw0:'+complexsuburl);
                   var complexsubcmd='';
                   console.log('lizw1:'+_this.data("complexname"));
                    var complexname =_this.data("complexname").split(',');
                    console.log('lizw2:'+complexname);
                    for(x in complexname ){
                    	console.log('lizw3:'+complexname[x])
                    	console.log('lizw3:'+obj.find("select[name=" + complexname[x] + "]").val())
                    	complexsubcmd += "&" + complexname[x]+'='+obj.find("select[name=" + complexname[x] + "]").val();
                    	
                    }
                    console.log('lizw2:'+complexsubcmd);
                    if (complexsuburl) {
                        if (tag) {
                            self.setSelectHtml(_thiscomplexSub, complexsuburl, complexsubcmd, valueCallback);
                        } else {
                            self.setSelectHtml(_thiscomplexSub, complexsuburl, complexsubcmd);
                        }
                    }
                }
            })
         ...
         *  @param cm [除搜索的条件，原来的条件]
         */
        treeSearchNew: function(ztreeObj, srvName, setting, ztreeSearchObj, searchKey, cm) {
            ...
            _btn.bind("click", function() {
                ...
                _value = $.trim(_value);
                if (cm) {
                    cmd = searchKey + '=' + _value + cm;
                } else {
                    cmd = searchKey + '=' + _value;
                }
                Rose.ajax.getJson(srvMap.get(srvName), cmd, function(json, status) {
                    if (status) {
        ...
        /**
         * [confirmModal 公用确认框]
         * @param  {Function} callback 回调函数，确认后执行的操作
         * @param  {[str]}   Message  [提示框内的内容]
         * @param  {[str]}   btn1     [按钮1的名称(取消按钮)]
         * @param  {[str]}   btn2     [按钮2的名称（确认按钮）]
         * @return {[type]}            [description]
         */
        confirmModal: function(callback, Message, btn1, btn2) {
            var _modal = $("#Modal_confirm");
            // 提示内容修设置
            if ((!Message) || Message == 'default') {
                _modal.find('.modal-body').find('h4').html("是否确认删除？")
            } else {
                _modal.find('.modal-body').find('h4').html(Message)
            }
            // 取消按钮设置
            if ((!btn1) || btn1 == 'default') {
                _modal.find("[name='N']").html("否")
            } else {
                _modal.find("[name='N']").html(btn1)
            }
            // 确认按钮设置
            if ((!btn2) || btn2 == 'default') {
                _modal.find("[name='Y']").html("是")
            } else {
                _modal.find("[name='Y']").html(btn2)
            }
            // 确认后回调
            _modal.modal('show');
         ...
         *
         */
        valideData: function(str, data, dataKey) {
            var strData = str.split(",");
            if (str == "") {
                $.each(data, function(index, val) {
                    /* iterate through array or object */
                    // str += "," + val[dataKey];
                    if (val) { str += val[dataKey] + ","; }
                });
        ...
        imgbox: function(obj, _src) {
            var mBox = XMS.dom.get("imgbox-overlay") || XMS.dom.createElementIn("div", document.body, false, {});
            var mWrap = XMS.dom.get("imgbox-bg-4") || XMS.dom.createElementIn("div", document.body, false, {
                className: "imgbox-wrap"
            });
            mWrap.id = "imgbox-bg-4";
            mBox.id = "imgbox-overlay";
            // 阻止冒泡
            ...
            obj.bind('click', function(event) {
                /* Act on the event */
                mBox.style.opacity = "0.5";
                var _id = $(this).parents("tr").find("[name=id]").val();
                var _href = srvMap.get(_src) + "?" + "id=" + _id + '&ver=' + new Date().getTime();
                // var _href = _src;
                var img = document.getElementById("imgbox-img-4")
                if (_href) {
                    getImageWidth(_href, function(w, h) {
                        // console.log({ width: w, height: h });
                        // console.log(document.body.clientHeight );
                        var _top = document.body.clientHeight / 2 - h / 2;
                        var _left = document.body.clientWidth / 2 - w / 2;
                        // 限制图片大小
                        // if (_top < 0 || _left < 0) {
                        //     w = w / 2;
                        //     h = h / 2;
                        //     _top = document.body.clientHeight / 2 - h / 2;
                        //     _left = document.body.clientWidth / 2 - w / 2;
                        // }
                        var _img_html = '<img class="imgbox-img" id="imgbox-img-4" alt="" src="' + _href + '">';
                        $("#imgbox-bg-4").html(_img_html);
                        // $("#imgbox-img-4").attr('src', _href);
                        $("#imgbox-bg-4").css({
                            width: w,
                            height: h,
                            top: _top,
                    ...
                    $("#imgbox-overlay").fadeIn('normal');
                    $("#imgbox-bg-4").fadeIn('normal');

                    $("#imgbox-overlay").unbind();
                    $("#imgbox-overlay").bind('click', function(event) {
                        /* Act on the event */
                        $("#imgbox-overlay").hide();
                        $("#imgbox-bg-4").hide();
                    });
                    $("#imgbox-bg-4").unbind();
                    $("#imgbox-bg-4").bind('click', function(event) {
                        /* Act on the event */
                        $("#imgbox-overlay").hide();
                        $("#imgbox-bg-4").hide();
                    });
